...
if [[ "$TEST" = "docs" ]]; then
  if [[ "$GITHUB_WORKFLOW" == "Cookbook CI" ]]; then
...
then
  ...
  then
    ...
    for item in $(jq -r '.versions[]| select(.component!="cookbook")| tojson' <<<"$REPORTED_STATUS")
    do
      echo $item
      COMPONENT="$(jq -r '.component' <<<"$item")"
      VERSION="$(jq -r '.version' <<<"$item")"
  ...
  then
    ...
    # Sadly: Different pulpcore-versions aren't either...
    for item in $(echo "$REPORTED_STATUS" | jq -r '.versions[]|select(.component!="cookbook")|(.package // ("pulp_" + .component)|sub("pulp_core"; "pulpcore"))|sub("-"; "_")')
    do
      ./generate.sh "${item}" python
      cmd_prefix pip3 install "/root/pulp-openapi-generator/${item}-client"
      sudo rm -rf "./${item}-client"
...
cmd_user_prefix bash -c "django-admin makemigrations cookbook --check --dry-run"

# Run unit tests.
cmd_user_prefix bash -c "PULP_DATABASES__default__USER=postgres pytest -v -r sx --color=yes --suppress-no-test-exit-code -p no:pulpcore --pyargs pulp_cookbook.tests.unit"
# Run functional tests
if [[ "$TEST" == "performance" ]]; then
  if [[ -z ${PERFORMANCE_TEST+x} ]]; then
    cmd_user_prefix bash -c "pytest -vv -r sx --color=yes --suppress-no-test-exit-code --capture=no --durations=0 --pyargs pulp_cookbook.tests.performance"
  else
    cmd_user_prefix bash -c "pytest -vv -r sx --color=yes --suppress-no-test-exit-code --capture=no --durations=0 --pyargs pulp_cookbook.tests.performance.test_${PERFORMANCE_TEST}"
  fi
  exit
fi
...
if [ -f "$FUNC_TEST_SCRIPT" ]; then
  ...
  then
    ...
    cmd_user_prefix bash -c "pytest -v -r sx --color=yes --suppress-no-test-exit-code --pyargs pulp_cookbook.tests.functional -m 'not parallel' --nightly"
  else
    ...
    cmd_user_prefix bash -c "pytest -v -r sx --color=yes --suppress-no-test-exit-code --pyargs pulp_cookbook.tests.functional -m 'not parallel'"
  fi 
fi
