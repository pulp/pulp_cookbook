...
# './plugin-template --travis pulp_cookbook' to update this file.
#
...
if [ "$TEST" = 'bindings' ]; then
  ...
  ./generate.sh pulp_cookbook ruby 0
...
if [[ "$TEST" == "performance" ]]; then
  ...
  if [[ -z ${PERFORMANCE_TEST+x} ]]; then
    travis-wait-enhanced --interval=1m --timeout=40m -- pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_cookbook.tests.performance
  else
    travis-wait-enhanced --interval=1m --timeout=40m -- pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_cookbook.tests.performance.test_$PERFORMANCE_TEST
  fi
  exit
fi
...
if [ -f $FUNC_TEST_SCRIPT ]; then
  source $FUNC_TEST_SCRIPT
    ...
    pytest -v -r sx --color=yes --pyargs pulp_cookbook.tests.functional
fi
