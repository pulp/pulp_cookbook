...
if [[ "$TEST" = "docs" ]]; then
  ...
  cmd_prefix bash -c "cd pulp_cookbook/docs; make diagrams html; tar -cvf docs.tar ./_build"
  docker cp pulp:/pulp_cookbook//docs/docs.tar docs.tar
...
cmd_prefix pip3 install /root/pulp-openapi-generator/pulp_cookbook-client
sudo rm -rf ./pulp_cookbook-client
