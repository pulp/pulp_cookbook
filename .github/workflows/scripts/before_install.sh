...
if [[ "$TEST" == "upgrade" ]]; then
  ...
  cp -R .ci /tmp/.ci
  git checkout $FROM_PULP_COOKBOOK_BRANCH
  rm -rf .ci .github
  cp -R /tmp/.github .
  cp -R /tmp/.ci .
fi
...
echo "legacy_component_name: pulp_cookbook" >> .ci/ansible/vars/main.yaml
echo "component_name: cookbook" >> .ci/ansible/vars/main.yaml
